...
See [Choosing a Source of Randomness](https://www.notion.so/Internal-Customer-Conduit-Oracle-VRF-HowTo-e7340ab9dc314b4ea89d4cd76aa26745?pvs=21) to help decide which is right for your application.

### Integrating VRF into a Smart Contract
...
- Once confirmed, we will `fulfillRandomness` on the contract that initiated the request
...
- The request must send a `msg.value` greater than or equal to the price of the call. See the [pricing](https://www.notion.so/Internal-Customer-Conduit-Oracle-VRF-HowTo-e7340ab9dc314b4ea89d4cd76aa26745?pvs=21) section for more details
...
First you have to deploy a `VRFCoordinatorV2Adapter` by calling this function on the `VRFCoordinatorV2AdapterFactory` :
...
contract ChainlinkDieRoller is VRFConsumerBaseV2 {
    ...
    function setVrf(address _vrf) public {
        vrf = VRFCoordinatorV2Interface(_vrf);
    }
